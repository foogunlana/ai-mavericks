...
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
    ...
    claude --chrome --model sonnet --effort low --permission-mode acceptEdits --verbose \
      --allowedTools 'Bash(bd *)' 'Bash(npm test*)' 'Bash(npm run typecheck*)' \
      --allowedTools 'Bash(npm run build:*)' 'Bash(npm run ui:smoke*)' \
      --allowedTools 'Bash(npm run start*)' 'Bash(npm run db:*)' \
      --allowedTools 'Bash(npx vitest*)' 'Bash(git *)' \
